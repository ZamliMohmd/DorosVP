...
@section('content')
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">مواضيع البحث الرئيسية</h3>
    ...
    border-color: #e08e0b;">
                                        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: bold">اضافة موضوع بحث رئيسي:</h4>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        <form action="{{url('/admin/mainspecialties/new')}}" method="POST" style="display: inline-block;">
                                            @csrf
                                            @method('POST')
    ...
    border-radius: 5px;">
                        ...
                        <h3 align="center">مجموع مواضيع البحث الرئيسية:<span id="total_records"></span></h3>
                    ...
                    <div class="box-body table-responsive no-padding" >
                        ...
                        @if ($mainTopics->count() > 0)
                            <table class="table table-hover">
                                ...
                                @foreach ($mainTopics as $key =>$specialty)
                                    <tr style="font-weight: bold">
                                        <td>{{$key+1 }}</td>
                                        <td>{{ $specialty->name_ar }} </td>
                                        <td>{{ $specialty->name_he}} </td>
                                        <td><a href="{{url('/admin/subtopics/'.$specialty->id.'/index')}}"><button class="btn btn-info">اضغط هنا</button> </a></td>
                        ...
                        @else
                            <div class="profile-content">
                                <div class="alert alert-warning">
                                    <p>لا توجد تخصصات رئيسية مضافة حاليا</p>
                                </div>
                            </div>
                        @endif
                    </div>
...
@section('footer')
    <script type="application/javascript">
        $(document).ready(function(){
            ...
            {
                $.ajax({
                    url:"{{ url('/admin/mainspecialties/search') }}",
                    method:'GET',
                    data:{query:query},
                    dataType:'json',
